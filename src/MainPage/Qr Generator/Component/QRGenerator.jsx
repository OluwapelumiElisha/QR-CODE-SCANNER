    ...
    <div className="bg-customColor h-screen overflow-auto">
      <div className="flex items-center justify-center pt-10">
        <div className="flex items-center justify-center w-60 h-60 rounded-lg border-2 border-orange-400 bg-gray-800">
          <img src={response?.data?.qrCode?.qrCodeData} alt="" />
        </div>
      </div>

      <div className="flex items-center justify-center mt-12">
        <Button onClick={handleGenerateqrcode} disabled={loading} className=" text-white font-pbold text-xl mt-4 bg-customYellow w-120 px-6 py-3 flex justify-center items-center">
      ...
      <div className="flex items-center justify-center mt-12">
        <h1 className="text-white text-2xl">{response?.data?.qrCode?.qrNumber}</h1>
      </div>
